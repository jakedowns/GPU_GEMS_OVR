...
//	less than or equal to 256.
	...
	void ConfigRandom()
	{
		...
		while(i < m_numBodies)
		{
			...
			positions[i] = new Vector4(pos.x*scale,pos.y*scale,pos.z*scale+m_zOffset,1.0f);
			velocities[i] = new Vector4(vel.x*vscale,vel.y*vscale,vel.z*vscale+m_zOffset,1.0f);
	...
	void ConfigExpand()
	{
		...
		while(i < m_numBodies) 
		{
			...
			positions[i] = new Vector4(pos.x*scale,pos.y*scale,pos.z*scale+m_zOffset,1.0f);
			velocities[i] = new Vector4(pos.x*vscale,pos.y*vscale,pos.z*vscale+m_zOffset,1.0f);
